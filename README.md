...
> provisioning, OpenML/HF jobs, artifacts, automatic tier routing, and
> multi-node Ray jobs.
...
- **[`examples/demo_notebook.ipynb`](examples/demo_notebook.ipynb)** --
  ...
  with explicit resources and a local artifact, automatic tier
  routing across compute clusters, and a multi-node Ray job. The best
  starting point if you want to see everything in action before
